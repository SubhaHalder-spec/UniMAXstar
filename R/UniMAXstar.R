#' UniMAXstar test for testing equality means against umbrella-ordered alternatives (unknown peak) in one-way ANOVA
#' @export
...
#' @details Testing of H_0:mu_1 = mu_2 = ... = mu_k vs H_1:mu_1 <=.....<= mu_(h-1)<= mu_h >= mu_(h+1)>=....>= mu_k (at least one strict inequality, h is unknown), where mu_i represents the population means of the i-th treatment. The input consists of two variables: sample_data and significance_level. The output consists of the peak of the umbrella, the critical value, the UniMAX* test statistic value, and the result, which indicates whether to reject or not reject the null hypothesis.
#' @importFrom Iso pava
#' @importFrom stats qchisq
#' @importFrom stats quantile rnorm var
#' @author Subha Halder
UniMAXstar <- function(sample_data, significance_level){
  set.seed(456)
  R_MLE <- function(X, n) {
    X1 <- X[-1]
    n1 <- n[-1]
  ...
  return(paste( "Peak:", peak2, "; Critical value:", quantile_value, "; UniMAX* Test statistic:", UMAX, "; Result:", result))
}
